...
worktree_window() {
  ...
  local branch="$1"
  if [ -z "$branch" ]; then
    echo "usage: worktree_window <branch-name> [base-branch] [plan-file]"
    return 1
  fi
  # どこからブランチを切るか（デフォルト：HEAD）
  local base="${2:-HEAD}"
  # プラン内容ファイル（指定時は新規ウィンドウでclaudeにそのまま渡して実装を開始する）
  local plan_file="$3"
  ...
  if [ -n "$TMUX" ]; then
    ...
    # プランファイルが指定されていれば、新規ウィンドウでclaudeをその内容を初期プロンプトとして起動する
    if [ -n "$plan_file" ]; then
      tmux send-keys -t "$branch" "claude \"\$(cat '${plan_file}')\"" Enter
    fi
  else
